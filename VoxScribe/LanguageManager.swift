...
class LanguageManager: ObservableObject {
    ...
    init() {
        let loadedLanguages: [SupportedLanguage] = Self.loadAvailableLanguages()
    ...
    private static func loadAvailableLanguages() -> [SupportedLanguage] {
        let locales: Set<Locale> = SFSpeechRecognizer.supportedLocales()
        
        return locales.sorted {
            let name1: String = $0.localizedString(forIdentifier: $0.identifier) ?? ""
            let name2: String = $1.localizedString(forIdentifier: $1.identifier) ?? ""
            return name1 < name2
        }.map { locale in
            let code: String = locale.identifier
            let name: String = locale.localizedString(forIdentifier: code) ?? code
    ...
    private static func autoDetectSystemLanguage(availableLanguages: [SupportedLanguage]) -> SupportedLanguage {
        ...
        if let match: SupportedLanguage = availableLanguages.first(where: { $0.code.lowercased() == systemCode }) {return match}
        // Try base language match (e.g. "en" if system is "en-GB")
        let baseCode: String = String(systemCode.prefix(2))
        if let match: SupportedLanguage = availableLanguages.first(where: { $0.code.lowercased().hasPrefix(baseCode) }) {return match}
    ...
    func setLanguage(code: String) {
        if let language = availableLanguages.first(where: { $0.code == code }) {self.selectedLanguage = language}
    }
}
...
struct SupportedLanguage: Identifiable, Hashable {
    ...
    let code: String   // Locale code eg: "en-US"
}
